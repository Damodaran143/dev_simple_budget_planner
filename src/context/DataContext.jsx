import { createContext, useCallback, useContext, useEffect, useMemo, useState } from 'react'
import { api } from '../utils/api'
import { useAuth } from './AuthContext'
import { currentMonthKey, monthKey } from '../utils/format'

const DataContext = createContext(null)

// DRF serializes Decimal fields as JSON strings (e.g. "80000.00"). The rest
// of this app does arithmetic directly on these values, so we normalize to
// numbers right where the data enters the app.
function normalizeEntry(e) {
  return { ...e, amount: Number(e.amount) }
}

function normalizeGoal(g) {
  return { ...g, targetAmount: Number(g.targetAmount) }
}

function normalizeBudgets(map) {
  const out = {}
  for (const [category, limit] of Object.entries(map)) out[category] = Number(limit)
  return out
}

export function DataProvider({ children }) {
  const { isAuthenticated } = useAuth()

  const [entries, setEntries] = useState([])
  const [goals, setGoals] = useState([])
  const [budgets, setBudgets] = useState({})
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState('')

  // Load everything fresh whenever a user signs in; clear it on sign-out.
  useEffect(() => {
    if (!isAuthenticated) {
      setEntries([])
      setGoals([])
      setBudgets({})
      return
    }

    let cancelled = false
    setIsLoading(true)
    setError('')

    Promise.all([api.get('/entries/'), api.get('/goals/'), api.get('/budgets/')])
      .then(([entriesData, goalsData, budgetsData]) => {
        if (cancelled) return
        setEntries(entriesData.map(normalizeEntry))
        setGoals(goalsData.map(normalizeGoal))
        setBudgets(normalizeBudgets(budgetsData))
      })
      .catch((err) => {
        if (!cancelled) setError(err.message || 'Could not load your data.')
      })
      .finally(() => {
        if (!cancelled) setIsLoading(false)
      })

    return () => {
      cancelled = true
    }
  }, [isAuthenticated])

  // ---- Transactions ----

  const addEntry = useCallback(async (entry) => {
    // Strip the client-side draft id (TransactionForm sets one) - the
    // server assigns the real id.
    const { id: _draftId, ...payload } = entry
    const created = await api.post('/entries/', payload)
    setEntries((prev) => [...prev, normalizeEntry(created)])
  }, [])

  const deleteEntry = useCallback(async (id) => {
    await api.delete(`/entries/${id}/`)
    setEntries((prev) => prev.filter((e) => e.id !== id))
  }, [])

  const sortedEntries = useMemo(() => {
    return [...entries].sort((a, b) => {
      if (a.date === b.date) return a.id < b.id ? -1 : 1
      return a.date < b.date ? -1 : 1
    })
  }, [entries])

  const totals = useMemo(() => {
    let income = 0
    let expense = 0
    for (const e of entries) {
      if (e.type === 'income') income += e.amount
      else expense += e.amount
    }
    return { income, expense, balance: income - expense }
  }, [entries])

  // ---- Savings goals ----
  // Contributions are just ledger withdrawals tagged with a goalId and the
  // "Savings" category, so a goal's saved amount is always derived from the
  // single source of truth (the ledger) rather than duplicated state.

  const addGoal = useCallback(async (goal) => {
    const created = await api.post('/goals/', goal)
    setGoals((prev) => [...prev, normalizeGoal(created)])
  }, [])

  const deleteGoal = useCallback(async (id) => {
    await api.delete(`/goals/${id}/`)
    setGoals((prev) => prev.filter((g) => g.id !== id))
  }, [])

  const contributeToGoal = useCallback(
    async (goalId, amount, note) => {
      const goal = goals.find((g) => g.id === goalId)
      if (!goal) return
      await addEntry({
        type: 'expense',
        category: 'Savings',
        goalId,
        date: new Date().toISOString().slice(0, 10),
        description: note?.trim() || `Saved for ${goal.name}`,
        amount,
      })
    },
    [goals, addEntry],
  )

  const goalsWithProgress = useMemo(() => {
    return goals.map((goal) => {
      const saved = entries
        .filter((e) => e.goalId === goal.id)
        .reduce((sum, e) => sum + e.amount, 0)
      const pct = goal.targetAmount > 0 ? Math.min(100, (saved / goal.targetAmount) * 100) : 0
      return { ...goal, saved, pct, isComplete: saved >= goal.targetAmount }
    })
  }, [goals, entries])

  // ---- Budgets ----

  const setBudget = useCallback(async (category, limit) => {
    const updated = await api.post('/budgets/', { category, limit })
    setBudgets(normalizeBudgets(updated))
  }, [])

  function spentByCategory(targetMonthKey = currentMonthKey()) {
    const map = {}
    for (const e of entries) {
      if (e.type !== 'expense') continue
      if (monthKey(e.date) !== targetMonthKey) continue
      map[e.category] = (map[e.category] || 0) + e.amount
    }
    return map
  }

  // Last N months of income/expense totals, oldest first — used by Reports.
  function monthlyTrend(months = 6) {
    const now = new Date()
    const buckets = []
    for (let i = months - 1; i >= 0; i--) {
      const d = new Date(now.getFullYear(), now.getMonth() - i, 1)
      const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`
      buckets.push({ key, income: 0, expense: 0 })
    }
    const byKey = Object.fromEntries(buckets.map((b) => [b.key, b]))
    for (const e of entries) {
      const k = monthKey(e.date)
      if (!byKey[k]) continue
      if (e.type === 'income') byKey[k].income += e.amount
      else byKey[k].expense += e.amount
    }
    return buckets
  }

  const resetAllData = useCallback(async () => {
    await api.post('/reset/')
    setEntries([])
    setGoals([])
    setBudgets({})
  }, [])

  const value = {
    entries: sortedEntries,
    addEntry,
    deleteEntry,
    totals,
    goals: goalsWithProgress,
    addGoal,
    deleteGoal,
    contributeToGoal,
    budgets,
    setBudget,
    spentByCategory,
    monthlyTrend,
    resetAllData,
    isLoading,
    error,
  }

  return <DataContext.Provider value={value}>{children}</DataContext.Provider>
}

export function useData() {
  const ctx = useContext(DataContext)
  if (!ctx) throw new Error('useData must be used within a DataProvider')
  return ctx
}
